...
export async function scrapForInternalPlayer(
  media_type: string,
  tmdb_id: string,
  episode?: string,
  season?: string
  ...
  try {
    let scrapData;
    // SCRAP ACORDING TO MEDIA TYPE
    if (media_type === "tv" || media_type === "anime") {
      scrapData = await axios.get(
        `https://hugo.vidlink.pro/api/tv/${tmdb_id}/${season}/${episode}?multiLang=0`
      );
    } else {
      scrapData = await axios.get(
        `https://hugo.vidlink.pro/api/movie/${tmdb_id}?multiLang=0`
      );
    }
    return {
      success: true,
