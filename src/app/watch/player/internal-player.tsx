...
const InternalPlayer: React.FC<Props> = ({
  scrapData,
  title,
  category,
  season,
  ...
  return (
    <div className="h-full w-full">
      <MediaPlayer
        title={
          category === "movie" ? title : `${title} - S${season}E${episode}`
        }
        src={url}
        autoPlay={true}
        ...
        ref={playerRef}
        onTimeUpdate={() =>
          handleCurrentTimeUpdate(playerRef.current?.currentTime)
        }
      ...
      >
        <MediaProvider>
          {tracks.length > 0 &&
            tracks.map((track: any) => <Track {...track} key={track.src} />)}
        </MediaProvider>
