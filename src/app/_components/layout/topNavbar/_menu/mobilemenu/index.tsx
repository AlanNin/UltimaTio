...
import { quitProfile } from "~/redux/profileSlice";
...
const MobileMenu: React.FC<Props> = ({
  ...
  return (
    <section
      id="!m_accountMenu"
      className="fixed inset-0 z-40 w-full h-full overflow-y-auto flex flex-col justify-between bg-[#0F0F0F]"
    >
      ...
      >
        ...
        >
          ...
          {!isChangeLanguageOpen ? (
            <ChevronRightIcon className="text-white h-[16px] w-auto stroke-current ml-[-20px]" />
          ) : (
            <ChevronDownIcon className="text-white h-[16px] w-auto stroke-current ml-[-20px]" />
          )}
        </div>
      </div>
